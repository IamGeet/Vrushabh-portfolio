    ...
    >
      <svg width="100%" height="100%" className="absolute inset-0">
        ...
        {shapes
          ...
          .map((s) => (
            <g key={`shape-${s.id}`} transform={`translate(${s.x}, ${s.y}) rotate(${s.rotation})`}>
              {currentSection === "skills" && (
                <polygon
                  points={`0,${-s.size} ${s.size * 0.866},${s.size * 0.5} ${-s.size * 0.866},${s.size * 0.5}`}
                  fill="none"
                  stroke={theme.accent}
                  strokeWidth="1"
                  opacity={s.opacity}
              ...
              {currentSection === "experience" && (
                <circle r={s.size / 2} fill="none" stroke={theme.primary} strokeWidth="1" opacity={s.opacity} />
              )}
            </g>
          ))}
      ...
      <div
        ...
        style={{
          backgroundImage: `radial-gradient(circle at ${mousePosition.x}px ${mousePosition.y}px, ${theme.primary}15 0%, transparent 50%)`,
        }}
      />
    </div>
  )
