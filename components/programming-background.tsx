...
const SeasonalParticles = React.memo(({ theme, scrollY }: { theme: SeasonalTheme; scrollY: number }) => {
  ...
  return (
    <g>
      {particles.filter((p) => isFiniteNumber(p.x) && isFiniteNumber(p.y)).map((p, idx) => getParticleShape(p, idx))}
    </g>
  )
})
  ...
  return (
    ...
    >
      <svg width="100%" height="100%" className="absolute inset-0">
        ...
        {isBrowser && isFiniteNumber(getWindowWidth() / 2) && isFiniteNumber(getWindowHeight() / 2) && (
          <circle
